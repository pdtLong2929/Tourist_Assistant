...
Changes vs. original:
  - init_pool() is called via lifespan in main.py (NOT at import time)
  - TransitService no longer pre-loads GTFS into RAM
  - _services dict is populated lazily on first request
"""
...
# Populated after DB pool is ready (called from main.py lifespan)
_services: Dict[str, TransitService] = {}


def init_services() -> None:
    """Called once from main.py lifespan after init_pool()."""
    _services["hcmc"] = TransitService("hcmc")
    _services["hn"] = TransitService("hn")
...
def get_all_stops(
    city: CityCode,
    skip: int = Query(0,   description="Skip the first N results (pagination)"),
    limit: int = Query(100, description="Maximum number of stops to return"),
):
