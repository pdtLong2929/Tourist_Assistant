...
from app.schema.schema import RecommendRequest
from app.services.assemble import recommend

logger = logging.getLogger(__name__)

RECOMMEND_ACTIONS = {"recommend", "recommend_vehicle", "vehicle_recommendation"}
...
def _format_recommendation(result: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "cars": [{"veh_id": car} for car in result.get("cars", [])],
        "bikes": [{"veh_id": bike} for bike in result.get("bikes", [])],
    }
...
def handle_recommendation_job(job: JobMessage) -> Dict[str, Any]:
    try:
        req = _validate_model(RecommendRequest, job.payload)
    except ValidationError as exc:
        raise ValueError(exc.errors()) from exc

    result = recommend(req.origin, req.destination, req.date, req.user)
    return _build_success(job, _format_recommendation(result))
...
def handle_job(payload: Any) -> Dict[str, Any]:
    ...
    if action in RECOMMEND_ACTIONS:
        return handle_recommendation_job(job)
...
def handle_pull_message(payload: Any) -> None:
    ...
    try:
        ...
        logger.info(
            "Handled Pub/Sub job job_id=%s action=%s",
            result.get("job_id"),
            result.get("action"),
        )
...
def start_pull_subscriber() -> Optional[threading.Thread]:
    ...
    thread = threading.Thread(
        ...
        name="pubsub-recommendation-subscriber",
        daemon=True,
    )
    thread.start()
